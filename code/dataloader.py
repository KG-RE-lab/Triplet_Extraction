...
class CustomDataLoader(object):
    ...
    def collate_fn_train(features):
        ...
        enr_table = [f.enr_table for f in features]
        enr_mask = [f.enr_mask for f in features]
        en_table = [f.en_table for f in features]
        en_mask = [f.en_mask for f in features]
        htr_table = [f.htr_table for f in features]
        htr_mask = [f.htr_mask for f in features]



        input_ids = sequence_padding(input_ids)
        input_mask = sequence_padding(input_mask)
        enr_table = matr_padding(enr_table)
        enr_mask = matr_padding(enr_mask)       
        en_table = mat_padding(en_table)
        en_mask = mat_padding(en_mask)
        htr_table = mat_padding(htr_table)
        htr_mask = mat_padding(htr_mask)

        tensors = [input_ids, input_mask, enr_table, enr_mask,en_table, en_mask, htr_table, htr_mask]
    ...
    def get_dataloader(self, data_sign):
        ...
        elif data_sign in ("train", "dev", "test", 'epo', 'seo', 'normal', '1', '2', '3', '4', '5'):
            datasampler = SequentialSampler(dataset)
